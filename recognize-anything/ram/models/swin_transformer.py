...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        # print("x", x.shape)
        if mask is not None:
            attn_bias = mask.to(dtype=x.dtype).unsqueeze(1).repeat(B_//mask.shape[0],1,1,1) + attn_bias
            # print("mask", mask.shape)
        # print("q", q.shape, "k", k.shape, "v", v.shape, "attn_bias", attn_bias.shape,  "relpos", relative_position_bias.shape)
